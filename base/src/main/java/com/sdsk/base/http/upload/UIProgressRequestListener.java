...
public abstract class UIProgressRequestListener implements UploadRequestBody.UploadListener {
            ...
            switch (msg.what) {
                case REQUEST_UPDATE:
                    ...
                    if (uiProgressRequestListener != null) {
                        ...
                        //回调抽象方法
                        uiProgressRequestListener.onUIRequestProgress(progressModel.getProgress(), progressModel.isDone());
                    }
                    break;
                default:
    ...
    @Override
    public void onRequestProgress(int progress, boolean done) {
        //通过Handler发送进度消息
        Message message = Message.obtain();
        message.obj = new ProgressModel(progress, done);
     ...
     * @param done 是否写入完成
     */
    public abstract void onUIRequestProgress(int progress, boolean done);
}
